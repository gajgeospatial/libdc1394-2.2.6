 ...
 */
LIBDC1394_API dc1394error_t dc1394_capture_setup(dc1394camera_t *camera, uint32_t num_dma_buffers, uint32_t flags);

/**
 * Stop the capture
 */
LIBDC1394_API dc1394error_t dc1394_capture_stop(dc1394camera_t *camera);

/**
 * Gets a file descriptor to be used for select(). Must be called after dc1394_capture_setup().
 */
LIBDC1394_API int dc1394_capture_get_fileno (dc1394camera_t * camera);
 ...
 */
LIBDC1394_API dc1394error_t dc1394_capture_dequeue(dc1394camera_t * camera, dc1394capture_policy_t policy, dc1394video_frame_t **frame);

/**
 * Returns a frame to the ring buffer once it has been used.
 */
LIBDC1394_API dc1394error_t dc1394_capture_enqueue(dc1394camera_t * camera, dc1394video_frame_t * frame);
 ...
 */
LIBDC1394_API dc1394bool_t dc1394_capture_is_frame_corrupt (dc1394camera_t * camera,
        dc1394video_frame_t * frame);

/**
 * Set a callback if supported by the platform (OS X only for now).
 */
LIBDC1394_API void dc1394_capture_set_callback (dc1394camera_t * camera,
